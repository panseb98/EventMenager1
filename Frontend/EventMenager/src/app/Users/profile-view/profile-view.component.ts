...
export class ProfileViewComponent implements OnInit {
  ...
  constructor(private eventService : EventService, private authService : AuthService, private dataPass : UserPassService, private route : ActivatedRoute, public dialog: MatDialog, private inviService : InvitationService) { }
  ...
  openDialog(): void {
    ...
    dialogRef.afterClosed().subscribe(result => {
     if(typeof result != 'undefined'){
      let newInvitation =  {
        userIdSender : +this.userId,
        userIdRept : +this.userIdLogged,
        eventId : result,
        invitationType : 'EVENT_INVITATION'
      } as AddInvitation;
      console.log(newInvitation);
      this.inviService.addInvitation(newInvitation).subscribe( res =>
        console.log('ok')
      );
